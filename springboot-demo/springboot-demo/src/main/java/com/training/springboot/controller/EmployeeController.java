...
public class EmployeeController {
  ...
  private List getAllEmployees() {return employeeService.getAllEmployees();}

  @GetMapping("/employees/{id}")
  private Employee getEmployeeById(@PathVariable("id") int id) {
    return employeeService.getEmployeeById(id);
  }
  @GetMapping("/employeeSal/{id}")
  private Employee salCheck(@PathVariable("id") int id) {
    if(employeeService.getEmployeeSalaryById(id)>employeeService.avgSalary()) {
      return employeeService.getEmployeeById(id);
    }
    else{
      throw new EmployeeInsufficientSalaryException("Insufficient Salary Exception occurred");
    }
  }
